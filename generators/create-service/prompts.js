...
const prompts = [
	{
        ...
        validate: input => {
        	...
        	if(!/^[a-z]+[a-z0-9\-_]*[a-z0-9]+$/.test(input)){
        		return chalk.red("工程名称只能包含字母、数字短线（-）或者下划线（_），且必须以英文字母开头，英文字母或数字结尾");
        	}
    		return true;
        }
	},{
        ...
        validate: input => {
        	...
        	if(!/^[a-z]+[a-z0-9\.]*[a-z0-9]+$/.test(input)){
                return chalk.red("Maven Group Id 只能包含小写英文、数字或点（.），且必须以英文字母开头，英文字母或数字结尾");
            }
        	return true;
        }
	},{
        ...
        validate: input => {
        	...
        	if(!/^[a-z]+[a-z0-9\-_]*[a-z0-9]+$/.test(input)){
        		return chalk.red("Maven Artifact Id 只能包含小写英文、数字、短线（-）或者下划线（_），且必须以英文字母开头，英文字母或数字结尾");
        	}
        	return true;
        }
	},{
        ...
        validate: input => {
            ...
            if(!/^[a-z]+[a-z0-9\.]*[a-z0-9]+$/.test(input)){
                return chalk.red("包名只能包含小写英文、数字或点（.），且必须以英文字母开头，英文字母或数字结尾");
            }
            return true;
        }
    },{
        ...
        when: props => props.configServerEnabled && props.configServerType == 'K8S',
        default: "default",
        validate: input => {
            if(!input) return chalk.red("命名空间名称不能为空");
            if(!/^[a-z]+[a-z0-9\-]*[a-z0-9]+$/.test(input)){
                return chalk.red("工程名称只能包含字母、数字短线（-），且必须以英文字母开头，英文字母或数字结尾");
            }
            return true;
        }
    },{
		type: "confirm",
		name: "tracingEnabled",
		message: props => `您的${serviceType(props)}是否需要调用链跟踪（Jagger）？`,
		default: true
	},{
		type: "confirm",
		...
		message: props => `请选择您的${serviceType(props)}要支持的消息队列类型：`,
		...
		type: "confirm",
		name: "configNow",
		message: props => `是否现在开始配置您的${serviceType(props)}（Consul、Jagger、MQ等）？`,
		default: true
	},{
		type: "input",
        name: "consulHost",
        ...
        validate: input => {
        	...
        	if(!/^[a-z0-9\-_\.]+$/.test(input)){
        		return chalk.red("Consul 地址只能包含小写英文、数字、短线（-）或者下划线（_）");
        	}
    		return true;
        }
	},{
        ...
        validate: input => {
        	if(!input) return true;
        	if(!/^[a-z0-9\-_\.]+$/.test(input)){
                return chalk.red("Jaeger 地址只能包含小写英文、数字、短线（-）或者下划线（_）");
            }
    		return true;
        }
	},{
        ...
        message: props => `请输入您的 ${props.messageQueueType == "RABBIT_MQ" ? "RabbitMQ":"Kafka"} 访问地址（域名或IP）：`,
        validate: (input,props) => {
        	if(!input) return true;
        	let mqType = props.messageQueueType == "RABBIT_MQ" ? "RabbitMQ":"Kafka";
        	if(!/^[a-z0-9\-_\.]+$/.test(input)){
                return chalk.red(`${mqType} 地址只能包含小写英文、数字、短线（-）或者下划线（_）`);
            }
    		return true;
        }
	},{
        ...
        default: "",
        when: props => props.configNow && props.messageQueueEnabled,
        message: props => `请输入您的 ${props.messageQueueType=="RABBIT_MQ"?"RabbitMQ":"Kafka"} 端口号：`,
        validate: (input,props) => {
        	if(!input) return true;
        	let mqType = props.messageQueueType == "RABBIT_MQ" ? "RabbitMQ":"Kafka";
        	if(!/^[0-9]+$/.test(input)){
        ...
        default: "",
        when: props => props.configNow && props.messageQueueEnabled && props.messageQueueType=="RABBIT_MQ",
        message: props => `请输入您的 ${props.messageQueueType=="RABBIT_MQ"?"RabbitMQ":"Kafka"} 用户名：`,
        validate: (input,props) => {
            if(!input) return true;
            let mqType = props.messageQueueType == "RABBIT_MQ" ? "RabbitMQ":"Kafka";
            if(!/^[a-z]+[a-z0-9\-_]*[a-z0-9]+$/.test(input)){
                return chalk.red(`${mqType} 用户名只能包含字母、数字短线（-）或者下划线（_），且必须以英文字母开头，英文字母或数字结尾`);
            }
            return true;
        }
    },{
        ...
        default: "",
        when: props => props.configNow && props.messageQueueEnabled && props.messageQueueType=="RABBIT_MQ",
        message: props => `请输入您的 ${props.messageQueueType=="RABBIT_MQ"?"RabbitMQ":"Kafka"} 密码：`,
        validate: (input,props) => {
            if(!input) return true;
            let mqType = props.messageQueueType == "RABBIT_MQ" ? "RabbitMQ":"Kafka";
